...
void calculateSceneProperties () {
	...
	for (int i = 0; i < floorVertex.size(); ++i) {
    	float ppdist = distPtoP(S_CENTER[0], S_CENTER[1], S_CENTER[2], floorVertex[i][0], floorVertex[i][1], floorVertex[i][2]);
    	if (SCENE_RADIUS < ppdist) SCENE_RADIUS = ppdist;
    }

    if (WALLS_VISIBLE) {
    	for (int i = 0; i < wallVertex.size(); ++i) {
	    	float ppdist = distPtoP(S_CENTER[0], S_CENTER[1], S_CENTER[2], wallVertex[i][0], wallVertex[i][1], wallVertex[i][2]);
	    	if (SCENE_RADIUS < ppdist) SCENE_RADIUS = ppdist;
	    }
    }
...
void copyArrayIntoArray(float arr2[4], float arr1[4]) {
...
void materialInitiation () {

	float snowColor[4] = {0.5,0.5,0.5,1};
	copyArrayIntoArray(snowColor, snowMaterial.ambient);
	copyArrayIntoArray(snowColor, snowMaterial.diffuse);
	copyArrayIntoArray(snowColor, snowMaterial.specular);
	snowMaterial.shininess = 10;

	//#FF7B0F
	float carrotColor[4] = {(float)0.5*0xFF/0xFF, (float)0.5*0x7B/0xFF, (float)0.5*0x0F/0xFF, 1};
	copyArrayIntoArray(carrotColor, carrotMaterial.ambient);
	copyArrayIntoArray(carrotColor, carrotMaterial.diffuse);
	copyArrayIntoArray(carrotColor, carrotMaterial.specular);
	carrotMaterial.shininess = 0;

	//054CFF
	float floorColor[4] = {(float)0x05/0xFF, (float)0x4C/0xFF, (float)0xFF/0xFF, 1};
	//float floorColor[4] = {0, 1, 0, 1};
	copyArrayIntoArray(floorColor, floorMaterial.ambient);
	copyArrayIntoArray(floorColor, floorMaterial.diffuse);
	copyArrayIntoArray(floorColor, floorMaterial.specular);
	floorMaterial.shininess = 100;

	//#059900
	float wallColor[4] = {(float)0x05/0xFF, (float)0x99/0xFF, (float)0x00/0xFF, 1};
	copyArrayIntoArray(wallColor, wallMaterial.ambient);
	copyArrayIntoArray(wallColor, wallMaterial.diffuse);
	copyArrayIntoArray(wallColor, wallMaterial.specular);
...
void renderSnowman () {
	...
	if (LIGHT_ENABLED) setMaterial(snowMaterial);
	...
	if (LIGHT_ENABLED) setMaterial(carrotMaterial);
...
const GLdouble floorNormal[3] = {0,1,0};
...
void renderScene (void) {
	...
	glTranslated(S_CENTER[0], S_CENTER[1], S_CENTER[2]);
	...
	float cameraPos[4] = {2,2,2,1};
	float cameraColor[4] = {1,1,1,1};
	glLightfv(GL_LIGHT1, GL_POSITION, cameraPos);
	glLightfv(GL_LIGHT1, GL_DIFFUSE, cameraColor);
	glLightfv(GL_LIGHT1, GL_SPECULAR, cameraColor);
	glLightfv(GL_LIGHT1, GL_AMBIENT, cameraColor);

	glPopMatrix();

    //Hero render
    for (int i = 0; i < gameobjects.size(); ++i) gameobjects[i].render(AXIS_VISIBLE, BOX_VISIBLE, LIGHT_ENABLED, NORMAL_PER_VERTEX);
	
	//Floor render
	setColor(0x37,0xA4,0x3E, 1);
	if (LIGHT_ENABLED) {
		setMaterial(floorMaterial);
		glNormal3dv(floorNormal);
	}
	...
	if (WALLS_VISIBLE) {
		...
		if (LIGHT_ENABLED) setMaterial(wallMaterial);
...
void keyboardEvent (unsigned char key, int x, int y) {
    ...
    switch((int)key) {
		...
		case N_KEY:
			NORMAL_PER_VERTEX = !NORMAL_PER_VERTEX;
			glutPostRedisplay();
			break;
