...
void GameModel::render (bool lightIsOn, bool normalPerVertex) {
	...
	for (int i = 0; i < faces.size(); ++i) {
		...
		if (lightIsOn && !normalPerVertex) glNormal3dv(f.normalC);

		for (int j = 0; j < f.v.size(); ++j) {

			if (lightIsOn && normalPerVertex) glNormal3dv(&_m.normals()[f.n[j]]);
